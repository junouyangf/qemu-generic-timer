...
static int hen_detect(BCM2835GpioState *s, int index)
{
    if (index >= 0 && index < 54) {
        return get_bit_2_u32(index, s->hen0, s->hen1);
    }
    return 0;
}
        ...
        uint32_t val, uint8_t start, uint8_t count, uint32_t *lev)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        }else if ((changes & cur) && hen_detect(s, start + i)) {
            /* If this is an input and must check high status */
            int irqline = 0;
            if (i > 27)
                irqline = 1;
            if (i > 45)
...
static void eds_clear(BCM2835GpioState *s, uint64_t value, int bank)
{
    ...
    if (hen_detect(s, gpio) && get_bit_2_u32(gpio, s->lev0, s->lev1) ){
    	return;
    }
    if (bank) {
        s->eds0 &= ~value;
