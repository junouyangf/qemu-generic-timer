    ...
    def Play(self):
        ...
        if( len(self.qemu_path)>0 and os.path.exists(self.qemu_path)):
            if  len(self.archivo) > 0  and os.path.exists(self.archivo):
                ...
                self.terminal_command = f'gnome-terminal -- {self.qemu_path} -M raspi3b  -kernel {self.archivo_kernel} -sd {self.archivo_img} -serial stdio -append "rw earlyprintk loglevel=8 console=ttyAMA0,115200 dwc_otg.lpm_enable=0 root=/dev/mmcblk0p2 rootdelay=1" -dtb  {self.archivo_dbt} '
        else:
            return 0
